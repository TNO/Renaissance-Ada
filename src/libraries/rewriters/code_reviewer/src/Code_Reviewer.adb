...
with Patchers_Append; use Patchers_Append;

procedure Code_Reviewer is

   Error_Count : Natural := 0;

   package Patchers_Vectors is new Ada.Containers.Vectors
     (Positive, Any_Constant_Patcher);
   ...
   Project_Filename : constant String :=
     Source_Directory & "\dependency_graph_extractor.gpr";
   ...
   begin
      for Unit of Units loop
         ...
         begin
            P.Marker.Mark (Current_Unit);
            P.Rewriter.Rewrite (Current_Unit);
   ...
   begin
      ...
      return Analyze_Project (Project_Filename, False);
   end Get_Units;
   ...
   function Get_Patchers return Patchers_Vectors.Vector is
      Vector : constant Patchers_Vectors.Vector :=
        Patchers_Vectors.Empty;
   begin
      Vector.Append (Patchers_Append.
   ...
   Example              : constant Analysis_Unit             :=
     Analyze_Fragment
       ("-- comment1" & ASCII.LF & "-- comment2" & ASCII.LF & "Name" &
        ASCII.LF & "-- comment3" & ASCII.LF & "-- comment4" & ASCII.LF,
        Expr_Rule);
