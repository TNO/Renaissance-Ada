...
package body Extraction.Project_Files is
   ...
   begin
      Graph.Write_Edge(Project, Project.Project_Path, Node_Edge_Types.Edge_Type_Source);

      if Recurse_Projects
        and then Extended_Project /= GPR.No_Project
        and then not Extended_Project.Externally_Built
      then
         Graph.Write_Edge(Project, Extended_Project, Node_Edge_Types.Edge_Type_Derives_From);
      end if;

      declare
         Iter            : GPR.Project_Iterator := GPR.Start(Project, Recurse_Projects, True);
      ...
      begin
         loop
            ...
            then
               Graph.Write_Edge(Project.Project_Path, Current_Project, Node_Edge_Types.Edge_Type_Imports);
            end if;
