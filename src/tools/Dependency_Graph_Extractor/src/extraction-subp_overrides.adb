   ...
   begin
      ...
      then
         declare
            ...
            Base_Decls : constant LAL.Basic_Decl_Array := Decl.P_Base_Subp_Declarations;
         begin
      ...
      then
         ...
         begin
            ...
            then -- Ignore implementations later in the same file.
               for Base_Decl of Base_Decls loop
                  if Base_Decl /= Decl
                  then -- Ignore Decl itself, which will be in the set, unless it is an implementation later in the same file.
                     Graph.Write_Edge(Base_Decl, Decl, Node_Edge_Types.Edge_Type_Is_Overridden_By);
                  end if;
               end loop;
            end if;
         end;
