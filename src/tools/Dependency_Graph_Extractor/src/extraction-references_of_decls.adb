...
package body Extraction.References_Of_Decls is
   ...
   begin
      if Parent.Kind not in LALCO.Ada_Dotted_Name then
         return Parent.Kind = LALCO.Ada_End_Name
   ...
   begin
      return Node.Kind in LALCO.Ada_Base_Id
        and then not Is_Node_In_End_Name(Node) -- End names are syntactic sugar.
        and then not Node.As_Name.P_Is_Defining
        and then not Utilities.Get_Referenced_Decl(Node.As_Name).Is_Null
        and then Utilities.Is_Relevant_Basic_Decl(Utilities.Get_Referenced_Decl(Node.As_Name));
   end Is_Relevant_Base_Id;

   function Is_Node_In_With_Clause(Node : LAL.Ada_Node'Class) return Boolean is
      Parent : constant LAL.Ada_Node := Node.Parent;
   begin
      if Parent.Kind not in LALCO.Ada_Dotted_Name then
         return Parent.Kind = LALCO.Ada_Name_List
   ...
   function Is_Node_In_Subunit_Name(Node : LAL.Ada_Node'Class) return Boolean is
      Parent : constant LAL.Ada_Node := Node.Parent;
   begin
      if Parent.Kind not in LALCO.Ada_Dotted_Name then
         return Parent.Kind = LALCO.Ada_Subunit
   ...
   function Is_Generic_Instantiation(Node : LAL.Ada_Node'Class) return Boolean is
     (Node.Kind in LALCO.Ada_Generic_Package_Instantiation | LALCO.Ada_Generic_Subp_Instantiation);

   function Get_Param_Instantiations(Basic_Decl : LAL.Basic_Decl'Class) return LAL.Param_Actual_Array is
   begin
      case LALCO.Ada_Basic_Decl(Basic_Decl.Kind) is
         when LALCO.Ada_Generic_Package_Instantiation =>
            return Basic_Decl.As_Generic_Package_Instantiation.F_Params.P_Zip_With_Params;
         when LALCO.Ada_Generic_Subp_Instantiation =>
            return Basic_Decl.As_Generic_Subp_Instantiation.F_Params.P_Zip_With_Params;
         when others =>
            raise Internal_Extraction_Error with "Cases in Is_Generic_Instantiation and Get_Param_Instantiations do not match";
      end case;
   ...
   begin
      if Is_Relevant_Base_Id(Node) then
         declare
            ...
            Target_Name : constant LAL.Defining_Name := Utilities.Get_Referenced_Defining_Name(Name);
            Target_Decl : constant LAL.Basic_Decl    := Utilities.Get_Referenced_Decl(Name);
         begin
         ...
         begin
            for Param of Get_Param_Instantiations(Instantiation) loop
               ...
               begin
                  ...
                  then
                     declare
                        Target_Name : constant LAL.Defining_Name := Actual.As_Defining_Name;
                        Target_Decl : constant LAL.Basic_Decl    := Utilities.Get_Parent_Basic_Decl(Actual);
                     begin
                        Graph.Write_Node(Target_Name, Target_Decl);
                     end;
                  end if;
   ...
   begin
      if Is_Relevant_Base_Id(Node)
        and then not Is_Node_In_With_Clause(Node) -- Handled by Extraction.With_Clauses
      then
         declare
            Name        : constant LAL.Name          := Node.As_Name;
            Target_Name : constant LAL.Defining_Name := Utilities.Get_Referenced_Defining_Name(Name);
         ...
         begin
            ...
            if not Is_Node_In_Subunit_Name(Name) then
               declare
                  Source_Decl : constant LAL.Basic_Decl := Utilities.Get_Parent_Basic_Decl(Name);
               begin
                  ...
                  then
                     Graph.Write_Edge(Source_Decl, Target_Name, Target_Decl, Node_Edge_Types.Edge_Type_References);
                  else
                     Graph.Write_Edge(Name.Unit, Target_Name, Target_Decl, Node_Edge_Types.Edge_Type_References);
                  end if;
               end;
            else
               Graph.Write_Edge(Name.Unit, Target_Name, Target_Decl, Node_Edge_Types.Edge_Type_References);
               ...
               begin
                  ...
                  then
                     declare
                        ...
                        Target_Decl : constant LAL.Basic_Decl    := Utilities.Get_Parent_Basic_Decl(Actual);
                     begin
                        Graph.Write_Edge(Instantiation, Target_Name, Target_Decl, Node_Edge_Types.Edge_Type_References);
                     end;
                  end if;
               end;
            end loop;
