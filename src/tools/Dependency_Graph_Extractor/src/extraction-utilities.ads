...
private package Extraction.Utilities is
   ...
   package Analysis_Unit_Vectors is new Ada.Containers.Vectors
     (Index_Type   => Positive,
      Element_Type => LAL.Analysis_Unit,
      "="          => LAL."=");

   package Project_Vectors is new Ada.Containers.Vectors
     (Index_Type   => Positive,
      Element_Type => GPR.Project_Type,
      "="          => GPR."=");
   ...
   -- names from the GNATPRO installation path, the installation path with be
   ...
   function Get_Unique_Filename
     (File             : VFS.Virtual_File;
      ...
      Make_Lower_Case  : Boolean)
      return String;
   -- Get a unique name for File. The name will be relative to Directory_Prefix
   -- if Directory_Prefix is a parent of File. For files from the GNATPRO
   -- installation path, the installation path with be replaced by
   -- `%GNATPRO_PATH%`.

   function Get_Referenced_Decl(Name : LAL.Name'Class) return LAL.Basic_Decl;

   function Get_Referenced_Defining_Name(Name : LAL.Name'Class) return LAL.Defining_Name;

   function Get_Parent_Basic_Decl(Node : LAL.Ada_Node'Class) return LAL.Basic_Decl;
...
private
   ...
   Standard_Unit_File     : constant VFS.Virtual_File := VFS.Create(+Standard_Unit_Filename, Normalize => True);
