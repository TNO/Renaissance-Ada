   ...
   begin
      ...
      then
         declare
            ...
            Primitives : constant LAL.Basic_Decl_Array := Type_Decl.P_Get_Primitives;
         begin
            -- Add non-standard subprograms that are primitive even if external.
   ...
   begin
      ...
      then
         ...
         begin
            for Basic_Decl of Primitives loop
               if Basic_Decl.Unit /= Basic_Decl.P_Standard_Unit then
                  Graph.Write_Edge(Basic_Decl, Type_Decl, Node_Edge_Types.Edge_Type_Is_Primitive_Subprogram_Of);
               end if;
            end loop;
         end;
      end if;
