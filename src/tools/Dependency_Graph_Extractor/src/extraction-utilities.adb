...
package body Extraction.Utilities is
   ...
   begin
      return Context : Project_Context do
         -- Note that the Unit_Provider becomes the owner of the Environment and
         -- Tree. Hence, we do not have to unload and free these ourselves. The
         -- Unit_Provider itself is reference counted and the Analysis_Context is
         -- a controlled type. Hence, we do not have to free these either.
         GPR.Initialize(Context.Project_Environment);
         Context.Project_Tree := new GPR.Project_Tree;
         Context.Project_Tree.Load(VFS.Create(+Project), Context.Project_Environment);
         Context.Unit_Provider := LALPP.Create_Project_Unit_Provider(Context.Project_Tree, Context.Project_Tree.Root_Project, Context.Project_Environment);
         Context.Analysis_Context := LAL.Create_Context(Unit_Provider => Context.Unit_Provider);
      end return;
   end Open_Project;
   ...
   is
      Iter    : GPR.Project_Iterator := Context.Project_Tree.Root_Project.Start(Recurse_Projects);
   ...
   is
      Files  : VFS.File_Array_Access := Context.Project_Tree.Root_Project.Source_Files(Recursive => Recurse_Projects);
   ...
   begin
      for File of Files.all loop
         ...
         begin
            if Is_Ada_File_In_Project_Context(Filename, Context, Recurse_Projects) then
               Result.Append(Context.Analysis_Context.Get_From_File(Filename));
            end if;
         end;
      end loop;
   ...
   function Is_Buildable_File_In_Project_Context
     (Filename : String;
      ...
      return Boolean
   is
      File_Info : constant GPR.File_Info := Context.Project_Tree.Info(VFS.Create(+Filename));
   begin
   ...
   function Is_Ada_File_In_Project_Context
     (Ada_Filename     : String;
      ...
      return Boolean
   is
      File_Info : constant GPR.File_Info := Context.Project_Tree.Info(VFS.Create(+Ada_Filename));
   begin
      if Recurse_Projects then
         return File_Info.Language = "ada"
   ...
   function Is_Project_Main_Program
     (Node    : LAL.Ada_Node'Class;
      ...
      return Boolean
   is
      File_Info : constant GPR.File_Info := Context.Project_Tree.Info(VFS.Create(+Node.Unit.Get_Filename));
   begin
      return File_Info.Project.Is_Main_File(+Node.Unit.Get_Filename)
   ...
   function Get_Unique_Filename
     (Filename         : String;
      ...
      return String
   is
      File : constant GNATCOLL.VFS.Virtual_File := GNATCOLL.VFS.Create(+Filename);
   begin
   ...
   function Get_GNATPRO_Directory_Prefix return VFS.Virtual_File is
      Gnatls_Path : VFS.Filesystem_String_Access := GNATCOLL.VFS_Utils.Locate_Exec_On_Path(+"gnatls");
   begin
   ...
   GNATPRO_Directory_Prefix  : constant VFS.Virtual_File := Get_GNATPRO_Directory_Prefix;
   ...
   is
      ...
      begin
         ...
         then
            return GNATPRO_Substitute_Prefix & GNAT.OS_Lib.Directory_Separator & (+File.Relative_Path(GNATPRO_Directory_Prefix));
   ...
   function Is_Subp_Body_Unit_With_Spec(Referenced_Decl : LAL.Basic_Decl) return Boolean is
   begin
      return Referenced_Decl.Parent.Kind = LALCO.Ada_Library_Item
   ...
   function Get_Referenced_Decl(Name : LAL.Name'Class) return LAL.Basic_Decl is
   ...
   begin
      ...
      -- In the case of a subprogram library unit use the specification and not
      -- the body when (a) the specification exists and (b) Name does not occur
      -- in the same unit as the body.
   ...
   function Get_Referenced_Defining_Name(Name : LAL.Name'Class) return LAL.Defining_Name is
      Referenced_Defining_Name : constant LAL.Defining_Name := Name.P_Referenced_Defining_Name;
      Referenced_Decl          : constant LAL.Basic_Decl    := Name.P_Referenced_Decl;
   begin
      ...
      if Is_Subp_Body_Unit_With_Spec(Referenced_Decl)
        and then Name.Unit.Root /= Referenced_Decl.Unit.Root
      then
         if Referenced_Decl.P_Previous_Part_For_Decl.P_Defining_Names'Length = 1 then
            return Referenced_Decl.P_Previous_Part_For_Decl.P_Defining_Name;
         else
            raise Internal_Extraction_Error with "Basic declaration with single defining name expected";
         end if;
      else
   ...
   function Get_Parent_Basic_Decl(Node : LAL.Ada_Node'Class) return LAL.Basic_Decl is
      Parent_Basic_Decl : LAL.Basic_Decl := Node.P_Parent_Basic_Decl;
   begin
      if Parent_Basic_Decl.Is_Null
      then
      ...
      while Parent_Basic_Decl.Kind = LALCO.Ada_Exception_Handler
      loop
         Parent_Basic_Decl := Parent_Basic_Decl.P_Parent_Basic_Decl;
      end loop;
   ...
   function Occurs_In_Local_Context(Basic_Decl : LAL.Basic_Decl) return Boolean is
      Parent : constant LAL.Basic_Decl           := Get_Parent_Basic_Decl(Basic_Decl);
      Kind   : constant LALCO.Ada_Node_Kind_Type := Parent.Kind;
   begin
      return Parent.P_Is_Subprogram
        or else (Kind = LALCO.Ada_Generic_Subp_Decl and then not Basic_Decl.P_Is_Formal) -- For consistency across formal parameters
        or else Kind = LALCO.Ada_Task_Body
        or else Kind = LALCO.Ada_Entry_Body;
   end Occurs_In_Local_Context;

   function Is_Relevant_Basic_Decl(Node : LAL.Ada_Node'Class) return Boolean is
      Kind : constant LALCO.Ada_Node_Kind_Type := Node.Kind;
   begin
      return Kind in LALCO.Ada_Basic_Decl
      -- Is nameless and, hence, cannot be referenced.
        and then Kind /= LALCO.Ada_Anonymous_Type_Decl
      -- Using children.
        and then Kind /= LALCO.Ada_Generic_Formal_Subp_Decl -- Children: Abstract_Formal_Subp_Decl and Concrete_Formal_Subp_Decl
        and then Kind /= LALCO.Ada_Generic_Formal_Type_Decl -- Child: Type_Decl
        and then Kind /= LALCO.Ada_Generic_Formal_Package   -- Child: Generic_Package_Instantiation
        and then Kind /= LALCO.Ada_Generic_Formal_Obj_Decl  -- Child: Object_Decl
      -- Using parent.
        and then Kind /= LALCO.Ada_Generic_Subp_Internal    -- Parent: Generic_Subp_Decl
        and then Kind /= LALCO.Ada_Generic_Package_Internal -- Parent: Generic_Package_Decl
        and then Kind /= LALCO.Ada_Single_Task_Type_Decl    -- Parent: Single_Task_Decl
        and then Kind /= LALCO.Ada_Exception_Handler        -- Using parent to avoid confusion
      ...
      -- Is non-interesting when local.
        and then (Kind /= LALCO.Ada_Object_Decl or else not Occurs_In_Local_Context(Node.As_Basic_Decl))
        and then (Kind /= LALCO.Ada_Number_Decl or else not Occurs_In_Local_Context(Node.As_Basic_Decl));
